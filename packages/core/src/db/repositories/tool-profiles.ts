 ...
 * Get profile by ID
 */
export async function getToolProfileById(
  db: DatabaseClient,
  profile_id: string
 ...
 * Get profile by name
 */
export async function getToolProfileByName(
  db: DatabaseClient,
  name: string
): Promise<ToolProfile | null> {
  const [profile] = await compatSelect(db)
...
export async function listToolProfiles(
  ...
  const results = await query.orderBy(tool_profiles.name).limit(limit + 1);
...
export async function getChildProfiles(
  ...
  return compatSelect(db).from(tool_profiles).where(eq(tool_profiles.inherited_from, profile_id));
}
...
export async function getRootProfiles(db: DatabaseClient): Promise<ToolProfile[]> {
  return compatSelect(db).from(tool_profiles).where(isNull(tool_profiles.inherited_from));
}
