...
export async function insertAuditEvent(
  db: DatabaseClient,
  event: Omit<NewAuditEvent, 'event_id'>
): Promise<void> {
...
export async function queryAuditEvents(
  ...
  const results = await query.orderBy(desc(audit_events.timestamp)).limit(limit + 1);
...
export async function countAuditEvents(
  ...
  let query = compatSelect(db, { count: sql<number>`count(*)`.as('count') }).from(audit_events);
...
export async function getAuditStatistics(
  ...
  return {
    ...
    by_client: by_client
      ...
      .map((r: any) => ({ client_id: r.client_id!, count: r.count })),
  };
}
