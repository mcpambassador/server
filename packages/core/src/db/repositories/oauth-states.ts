 ...
 */
export async function getOAuthState(db: DatabaseClient, state: string): Promise<OAuthState | null> {
...
export async function consumeOAuthState(
  ...
  return compatTransaction(db, async () => {
    ...
    const [stateRecord] = await compatSelect(db)
      .from(oauth_states)
      .where(and(eq(oauth_states.state, state), gt(oauth_states.expires_at, now)))
      .limit(1);
