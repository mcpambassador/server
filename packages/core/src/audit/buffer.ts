...
export class AuditBuffer {
  ...
  constructor(config: AuditBufferConfig, flushCallback: FlushCallback) {
    ...
    logger.info(`[audit-buffer] Initialized with size=${config.size}, flush_interval=${config.flush_interval_ms}ms, spill=${config.spill_to_disk}, max_spill=${this.maxSpillSize}`);
  }
  ...
  async flush(): Promise<void> {
    ...
    try {
      ...
      if (error instanceof Error) {
        throw new AmbassadorError(`Failed to flush audit events: ${error.message}`, 'audit_flush_error');
      }
      throw error;
    }
  }
  ...
  private async spillEvent(event: AuditEvent): Promise<void> {
    try {
      ...
      if (this.spillFileSize >= this.maxSpillSize) {
        logger.warn(`[audit-buffer] Spill file size limit reached (${this.maxSpillSize} bytes) - switching to drop mode`);
        this.stats.total_dropped++;
        return;
      }
