...
--> statement-breakpoint
CREATE INDEX `idx_preshared_keys_key_prefix` ON `preshared_keys` (`key_prefix`);
--> statement-breakpoint
CREATE INDEX `idx_preshared_keys_user_id` ON `preshared_keys` (`user_id`);
--> statement-breakpoint
CREATE INDEX `idx_preshared_keys_status` ON `preshared_keys` (`status`);
--> statement-breakpoint
CREATE UNIQUE INDEX `unique_preshared_keys_key_hash` ON `preshared_keys` (`key_hash`);
...
--> statement-breakpoint
CREATE UNIQUE INDEX `unique_user_sessions_token_hash` ON `user_sessions` (`session_token_hash`);
...
--> statement-breakpoint
CREATE INDEX `idx_session_connections_session_id` ON `session_connections` (`session_id`);
--> statement-breakpoint
CREATE INDEX `idx_session_connections_status` ON `session_connections` (`status`);
