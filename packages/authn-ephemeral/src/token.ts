...
import type { ValidatedClient, GeneratedSessionToken, VerifiedSession } from './types.js';
...
export async function validateClientKey(
  ...
  const candidates = await db.query.clients.findMany({
    where: (keys, { eq, and }) => and(eq(keys.key_prefix, keyPrefix), eq(keys.status, 'active')),
  });

  // M-002 fix: Filter out expired keys (expires_at is nullable — null means no expiry)
  const now = new Date().toISOString();
  const validCandidates = candidates.filter(c => !c.expires_at || c.expires_at > now);
  ...
  for (const candidate of validCandidates) {
    try {
      ...
      logger.debug(
        `[authn-ephemeral] Argon2 verification error: ${err instanceof Error ? err.message : 'unknown'}`
      );
    }
  }
 ...
 */
export function generateSessionToken(hmacSecret: Buffer, sessionId: string): GeneratedSessionToken {
