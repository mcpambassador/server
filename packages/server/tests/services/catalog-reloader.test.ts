...
import {
  CatalogReloader,
  CatalogReloadConflictError,
} from '../../src/services/catalog-reloader.js';
...
describe('CatalogReloader (unit)', () => {
  ...
  it('diff algorithm - update', async () => {
    ...
    const desiredFp = computeConfigFingerprint(
  ...
  it('per-user diff detection', async () => {
    const shared = [] as any[];
    const perUser = [
      makeEntry('u-b', 'stdio', { command: ['1'] }, 'per_user'),
      makeEntry('u-c', 'stdio', { command: ['2'] }, 'per_user'),
    ];
    (listMcpCatalogEntries as any).mockResolvedValueOnce({ entries: shared, has_more: false });
