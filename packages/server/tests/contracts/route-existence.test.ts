...
async function gatherSpaRoutes(): Promise<Array<{ method: string; path: string }>> {
  ...
  const files: string[] = [];
  try {
    collectFiles(spaRoot, files);
  } catch (e) {
    /* ignore if spa not present */
  }
  ...
  it('all SPA-referenced API endpoints must be registered on the server', async () => {
      ...
      const isRouteNotFound =
        (body &&
          (body.code === 'FST_ERR_NOT_FOUND' ||
            (typeof body.message === 'string' && body.message.includes('not found')))) ||
        (!body && res.statusCode === 404);
      if (isRouteNotFound) {
    ...
    if (missing.length) {
      const lines = missing.map(
        m => `${m.method} ${m.path} => status ${m.status} body=${JSON.stringify(m.body)}`
      );
