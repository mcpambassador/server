...
import { startTestContainer, stopTestContainer, TEST_BASE_URL, getContainerLogs } from './setup';
import {
  makeRequest,
  registerClient,
  fetchTools,
...
function adminRequest<T>(
  method: string,
  path: string,
  options: { body?: any; expectStatus?: number } = {}
): Promise<T> {
      ...
      res => {
        ...
        res.on('end', () => {
          if (options.expectStatus && res.statusCode !== options.expectStatus) {
            return reject(
              new Error(`Expected ${options.expectStatus}, got ${res.statusCode}: ${data}`)
            );
          }
          try {
...
describe('Phase 2 Validation (M11) - E2E', () => {
  ...
  describe('M11.7 - Kill Switch End-to-End', () => {
    ...
    (hasAdmin ? it : it.skip)('admin toggles kill switch and client observes changes', async () => {
      ...
      await adminRequest('POST', `/v1/admin/kill-switch/${sampleTool}`, {
        body: { enabled: true },
        expectStatus: 200,
      });
  ...
  describe('M11.8 - Audit Completeness', () => {
    ...
    (hasAdmin ? it : it.skip)(
      'audit events include registration, tool_fetch, invocation, kill_switch, and admin_login',
      async () => {
        // Query events - admin endpoint
        const resp = await adminRequest<{ events: any[] }>('GET', '/v1/audit/events', {
          expectStatus: 200,
        });
  ...
  describe('Security Acceptance (SEC-M11-01..03)', () => {
    ...
    (Boolean(ADMIN_API_KEY) ? it : it.skip)(
      'SEC-M11-03: admin login sets secure, HttpOnly session cookie (basic check)',
      async () => {
