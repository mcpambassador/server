...
describe('Phase 3 Group Access Control', () => {
  ...
  beforeAll(async () => {
    ...
    const uA = await createUser(server.db, {
      username: 'groupAuser',
      password: 'pass1234',
      display_name: 'Group A',
    });
    userAId = uA.user_id;
    const uB = await createUser(server.db, {
      username: 'groupBuser',
      password: 'pass1234',
    ...
    // Create groups
    const gA = await server.fastify.inject({
      method: 'POST',
      url: '/v1/admin/groups',
      headers: { 'X-Admin-Key': server.adminKey },
      payload: { name: 'grp-A-' + Date.now(), status: 'active' },
    });
    groupA = JSON.parse(gA.body).data.group_id;
    const gB = await server.fastify.inject({
      method: 'POST',
      url: '/v1/admin/groups',
      headers: { 'X-Admin-Key': server.adminKey },
      payload: { name: 'grp-B-' + Date.now(), status: 'active' },
    });
    groupB = JSON.parse(gB.body).data.group_id;

    // Create and publish MCPs
    const mA = await server.fastify.inject({
      method: 'POST',
      url: '/v1/admin/mcps',
      headers: { 'X-Admin-Key': server.adminKey },
      payload: {
    ...
    await server.fastify.inject({
      method: 'POST',
      url: `/v1/admin/mcps/${mcpA}/validate`,
      headers: { 'X-Admin-Key': server.adminKey },
    });
    await server.fastify.inject({
      method: 'POST',
      url: `/v1/admin/mcps/${mcpA}/publish`,
      headers: { 'X-Admin-Key': server.adminKey },
    });

    const mBres = await server.fastify.inject({
      method: 'POST',
      url: '/v1/admin/mcps',
      headers: { 'X-Admin-Key': server.adminKey },
      payload: {
    ...
    await server.fastify.inject({
      method: 'POST',
      url: `/v1/admin/mcps/${mcpB}/validate`,
      headers: { 'X-Admin-Key': server.adminKey },
    });
    await server.fastify.inject({
      method: 'POST',
      url: `/v1/admin/mcps/${mcpB}/publish`,
      headers: { 'X-Admin-Key': server.adminKey },
    });
    ...
    await server.fastify.inject({
      method: 'POST',
      url: `/v1/admin/groups/${groupA}/mcps`,
      headers: { 'X-Admin-Key': server.adminKey },
      payload: { mcp_id: mcpA },
    });
    ...
    await server.fastify.inject({
      method: 'POST',
      url: `/v1/admin/groups/${groupB}/mcps`,
      headers: { 'X-Admin-Key': server.adminKey },
      payload: { mcp_id: mcpB },
    });
    ...
    await server.fastify.inject({
      method: 'POST',
      url: `/v1/admin/groups/${groupA}/members`,
      headers: { 'X-Admin-Key': server.adminKey },
      payload: { user_id: userAId },
    });

    // Add userB to groupB
    await server.fastify.inject({
      method: 'POST',
      url: `/v1/admin/groups/${groupB}/members`,
      headers: { 'X-Admin-Key': server.adminKey },
      payload: { user_id: userBId },
    });

    // Login both users
    const lA = await server.fastify.inject({
      method: 'POST',
      url: '/v1/auth/login',
      payload: { username: 'groupAuser', password: 'pass1234' },
    });
    const cA = lA.headers['set-cookie'];
    userAcookie = Array.isArray(cA) ? cA[0].split(';')[0] : (cA as string).split(';')[0];

    const lB = await server.fastify.inject({
      method: 'POST',
      url: '/v1/auth/login',
      payload: { username: 'groupBuser', password: 'pass1234' },
    });
  ...
  it('Removing user from group removes access', async () => {
    ...
    await server.fastify.inject({
      method: 'DELETE',
      url: `/v1/admin/groups/${groupA}/members/${userAId}`,
      headers: { 'X-Admin-Key': server.adminKey },
    });
  ...
  it('Removing MCP from group removes access', async () => {
    ...
    await server.fastify.inject({
      method: 'DELETE',
      url: `/v1/admin/groups/${groupA}/mcps/${mcpA}`,
      headers: { 'X-Admin-Key': server.adminKey },
    });
