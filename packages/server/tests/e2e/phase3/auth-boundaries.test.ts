...
describe('Phase 3 Auth Boundaries', () => {
  ...
  beforeAll(async () => {
    ...
    const u = await createUser(server.db, {
      username: 'authbounduser',
      password: 'pass1234',
      display_name: 'AuthBound',
      email: 'authbound@example.com',
    });
    userId = u.user_id;
    ...
    const login = await server.fastify.inject({
      method: 'POST',
      url: '/v1/auth/login',
      payload: { username: 'authbounduser', password: 'pass1234' },
    });
    const cookies = login.headers['set-cookie'];
    userCookie = Array.isArray(cookies)
      ? cookies[0].split(';')[0]
      : (cookies as string).split(';')[0];
  });
  ...
  it("User cannot access another user's clients", async () => {
    ...
    const u2 = await createUser(server.db, {
      username: 'authbound2',
      password: 'pass1234',
      display_name: 'AuthBound2',
    });
    ...
    const login2 = await server.fastify.inject({
      method: 'POST',
      url: '/v1/auth/login',
      payload: { username: 'authbound2', password: 'pass1234' },
    });
    const cookie2 = login2.headers['set-cookie'];
    const session2 = Array.isArray(cookie2)
      ? cookie2[0].split(';')[0]
      : (cookie2 as string).split(';')[0];

    const createClient = await server.fastify.inject({
      method: 'POST',
      url: '/v1/users/me/clients',
      headers: { cookie: session2 },
      payload: { client_name: 'ab-client', profile_id: profile.profile_id },
    });
    ...
    const res = await server.fastify.inject({
      method: 'GET',
      url: `/v1/users/me/clients/${clientId}`,
      headers: { cookie: userCookie },
    });
  ...
  it('User only sees MCPs from their groups', async () => {
    ...
    const createGroup = await server.fastify.inject({
      ...
      payload: { name: 'authbound-group-' + Date.now(), status: 'active' },
    });
    ...
    await server.fastify.inject({
      method: 'POST',
      url: `/v1/admin/mcps/${mid}/validate`,
      headers: { 'X-Admin-Key': server.adminKey },
    });
    await server.fastify.inject({
      method: 'POST',
      url: `/v1/admin/mcps/${mid}/publish`,
      headers: { 'X-Admin-Key': server.adminKey },
    });
    await server.fastify.inject({
      method: 'POST',
    ...
    const mk = await server.fastify.inject({
      method: 'GET',
      url: '/v1/marketplace',
      headers: { cookie: userCookie },
    });
