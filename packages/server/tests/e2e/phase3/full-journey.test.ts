...
describe('Phase 3 Full Journey (E2E)', () => {
  ...
  it('User logs in and gets session', async () => {
    ...
    const userRow = await server.db.query.users.findFirst({
      where: (u, { eq }) => eq(u.user_id, createdUserId),
    });
    ...
    // Get session
    const sessionRes = await server.fastify.inject({
      method: 'GET',
      url: '/v1/auth/session',
  ...
  it('Admin creates group and adds user', async () => {
    const createGroup = await server.fastify.inject({
      ...
      headers: { 'X-Admin-Key': server.adminKey },
      payload: {
        name: 'phase3-group-' + Date.now(),
        description: 'Phase3 test group',
        status: 'active',
      },
    });
  ...
  it('Admin creates, validates, publishes MCP and assigns to group', async () => {
    ...
    const validate = await server.fastify.inject({
      method: 'POST',
      url: `/v1/admin/mcps/${createdMcpId}/validate`,
      headers: { 'X-Admin-Key': server.adminKey },
    });
    expect(validate.statusCode).toBe(200);

    const publish = await server.fastify.inject({
      method: 'POST',
      url: `/v1/admin/mcps/${createdMcpId}/publish`,
      headers: { 'X-Admin-Key': server.adminKey },
    });
  ...
  it('User creates client and subscribes to MCP, selects tools, manages credentials and changes password', async () => {
    ...
    const mcp = await server.db.query.mcp_catalog.findFirst({
      where: (m, { eq }) => eq(m.mcp_id, createdMcpId),
    });
  ...
  it('Admin archives MCP and it disappears from marketplace', async () => {
    const arc = await server.fastify.inject({
      method: 'POST',
      url: `/v1/admin/mcps/${createdMcpId}/archive`,
      headers: { 'X-Admin-Key': server.adminKey },
    });
    expect(arc.statusCode).toBe(200);

    const mk2 = await server.fastify.inject({
      method: 'GET',
      url: '/v1/marketplace',
      headers: { cookie: sessionCookie },
    });
