...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function mainRequest<T = any>(method: string, path: string, body?: unknown): Promise<HttpResponse<T>> {
...
describe('M18 E2E - Admin Users / Preshared Keys / Sessions', () => {
  ...
  it('T3: Session List + Terminate', async () => {
    ...
    const sessions = ses.body?.data ?? ses.body?.sessions ?? (Array.isArray(ses.body) ? ses.body : []);
  ...
  it('T4: Integration — Create User → Key → Register Session → Suspend → Session Expired', async () => {
    ...
    await waitForCondition(async () => {
      ...
      return arr.some(
        (x: Record<string, unknown>) =>
          x.session_id === sessionId || x.user_id === userId,
      );
    }, 5000, 250);
    ...
    await waitForCondition(async () => {
      ...
      const entry = arr.find(
        (x: Record<string, unknown>) => x.user_id === userId,
      );
