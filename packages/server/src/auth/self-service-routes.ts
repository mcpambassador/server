...
export async function registerSelfServiceRoutes(
   ...
   */
  fastify.get('/v1/users/me', { preHandler: requireUserSession }, async (request, reply) => {
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (!validUser) {
          return reply
            .status(401)
            .send(wrapError(ErrorCodes.UNAUTHORIZED, 'Current password is incorrect'));
        }
        ...
        if (err instanceof ZodError) {
          return reply
            .status(400)
            .send(wrapError(ErrorCodes.VALIDATION_ERROR, 'Invalid request body', err.errors));
        }

        fastify.log.error({ err }, '[Auth] Password change error');
        return reply
          .status(500)
          .send(wrapError(ErrorCodes.INTERNAL_ERROR, 'Failed to change password'));
      }
    }
  );
}
