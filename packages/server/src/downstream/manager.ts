...
export class DownstreamMcpManager {
  ...
  async initialize(configs: DownstreamMcpConfig[]): Promise<void> {
    ...
    const startPromises = configs.map(async (config) => {
      try {
        ...
        if (config.transport === 'stdio') {
          ...
          console.warn(`[DownstreamMcpManager] Transport ${config.transport} not yet implemented for ${config.name}`);
        }
      } catch (err) {
    ...
    console.log(`[DownstreamMcpManager] Initialized ${this.connections.size} connections, ${this.aggregatedTools.length} tools`);
  }
  ...
  private async aggregateTools(): Promise<void> {
    ...
    console.log(`[DownstreamMcpManager] Aggregated ${this.aggregatedTools.length} tools from ${this.connections.size} MCPs`);
  }
  ...
  async healthCheckAll(): Promise<ConnectionHealth[]> {
    const healthChecks = Array.from(this.connections.values()).map(conn =>
      conn.healthCheck()
    );
