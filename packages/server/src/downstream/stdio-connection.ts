...
export class StdioMcpConnection extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    childProcess.stderr?.on('data', (data: Buffer) => {
      ...
      const truncated = redacted.length > 500 
  ...
  private handleStdout(data: Buffer): void {
    ...
    for (const line of lines) {
      ...
      try {
        ...
        console.error(`[MCP:${this.config.name}] Failed to parse message:`, line.substring(0, 100), err);
      }
    }
  }
  ...
  private redactCredentials(text: string): string {
    return text
      ...
      .replace(/(token|password|secret|key|apikey|api_key)[\s]*[:=][\s]*["']?[^\s"',}]+/gi, '$1=***REDACTED***');
  }
