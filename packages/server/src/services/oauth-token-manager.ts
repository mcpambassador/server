...
export class OAuthTokenManager {
  ...
  async generateAuthorizationUrl(
    ...
    console.log(`[OAuthTokenManager] Generating authorization URL for user=${userId}, mcp=${mcpId}`);
    ...
    if (oauthConfig.extra_params) {
      const blocklist = new Set(['client_id', 'response_type', 'redirect_uri', 'state', 'code_challenge', 'code_challenge_method', 'scope']);
      for (const [key, value] of Object.entries(oauthConfig.extra_params)) {
        if (!blocklist.has(key.toLowerCase())) {
          ...
          console.warn(`[OAuthTokenManager] Blocked extra_param '${key}' — reserved OAuth parameter`);
        }
      }
    }
    ...
    console.log(`[OAuthTokenManager] Generated authorization URL with state=${state.substring(0, 8)}...`);
  ...
  async exchangeCodeForTokens(
    ...
    console.log(`[OAuthTokenManager] Exchanging code for tokens with state=${state.substring(0, 8)}...`);
    ...
    try {
      ...
      for (const { token, hint } of tokensToRevoke) {
        try {
          ...
          if (!response.ok) {
            console.log(`[OAuthTokenManager] Token revocation returned ${response.status} for ${hint}`);
          } else {
