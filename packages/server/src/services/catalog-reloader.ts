...
export class CatalogReloader {
  ...
  async previewChanges(): Promise<PendingChanges> {
    ...
    const desiredShared = new Map(sharedResult.entries.map(e => [e.name, computeFingerprint(e)]));
  ...
  private async _doReload(): Promise<ReloadResult> {
    ...
    const perUserChanges = this.userPool.updateMcpConfigs(
      newPerUserConfigs,
      newPerUserFingerprints
    );
