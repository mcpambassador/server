/* eslint-disable no-console, @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-explicit-any */
    ...
    try {
      ...
      if (error.code === 'EEXIST') {
        ...
        if (!retriedFilesExist.every(Boolean)) {
          throw new Error('[TLS] Certificate generation timeout or failure by parallel instance', { cause: error });
        }
