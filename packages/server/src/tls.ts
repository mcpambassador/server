...
async function generateCA(
  ...
  await execCommand('openssl', ['genrsa', '-out', caKeyPath, '2048']);
...
async function generateServerCert(
  ...
  await execCommand('openssl', ['genrsa', '-out', serverKeyPath, '2048']);
...
export async function initializeTls(config: TlsConfig): Promise<TlsCertificates> {
  ...
  if (!filesExist.every(Boolean)) {
    ...
    try {
      ...
      logger.info('[TLS] Generating self-signed CA and server certificate...');
      ...
      if (error.code === 'EEXIST') {
        ...
        logger.info('[TLS] Certificate generation in progress by another instance, waiting...');
        ...
        logger.info('[TLS] Using certificates generated by parallel instance');
      } else {
        // Unexpected error
        if (lockHandle) {
    ...
    logger.info('[TLS] Using existing certificates');
  }
