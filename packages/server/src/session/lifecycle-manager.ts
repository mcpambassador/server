...
export class SessionLifecycleManager {
  ...
  private async checkExpiredSessions(nowIso: string): Promise<void> {
    const expiredSessions = await this.db.query.user_sessions.findMany({
      where: (sessions, { lt, ne }) =>
        and(lt(sessions.expires_at, nowIso), ne(sessions.status, 'expired')),
    });
    ...
    for (const session of expiredSessions) {
      ...
      if (this.userPool && session.user_id) {
        try {
          ...
          logger.info(
            `[SessionLifecycle] Terminated per-user MCPs for expired user ${session.user_id}`
          );
        } catch (err) {
          logger.error(
            { err, userId: session.user_id },
            '[SessionLifecycle] Failed to terminate per-user MCPs on expiry'
          );
  ...
  private async checkIdleSessions(nowMs: number, nowIso: string): Promise<void> {
    ...
    for (const session of idleSessions) {
      ...
      if (idleDurationMs > spindownThresholdMs) {
        logger.info(
          `[SessionLifecycle] Session ${session.session_id} transitioning to spinning_down`
        );
    ...
    for (const session of spinningDownSessions) {
      ...
      if (this.userPool && session.user_id) {
        try {
          ...
          logger.error(
            { err, userId: session.user_id },
            '[SessionLifecycle] Failed to terminate per-user MCPs'
          );
