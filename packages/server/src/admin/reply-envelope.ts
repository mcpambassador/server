...
import type { SuccessEnvelope, PaginatedEnvelope, ErrorEnvelope } from '@mcpambassador/contracts';
 ...
 */
export function wrapPaginated<T>(data: T[], pagination: PaginationMetadata): PaginatedEnvelope<T> {
 ...
 */
export function wrapError(code: string, message: string, details?: unknown[]): ErrorEnvelope {
  return {
    ok: false,
    error: {
      code,
