    ...
    try {
      ...
      if (message.includes('Registry not loaded')) {
        return reply.status(502).send(
          wrapError(ErrorCodes.INTERNAL_ERROR, 'Registry not available. Try refreshing.')
        );
      }
      ...
      try {
        ...
        if (!entry) {
          return reply.status(404).send(
            wrapError(ErrorCodes.NOT_FOUND, `MCP '${params.name}' not found in registry`)
          );
        }
        ...
        if (!result.success) {
          ...
          if (result.message.includes('already installed')) {
            return reply.status(409).send(
              wrapError(ErrorCodes.CONFLICT, result.message)
            );
          }
          ...
          if (result.message.includes('not found in registry')) {
            return reply.status(404).send(
              wrapError(ErrorCodes.NOT_FOUND, result.message)
            );
          }

          // Other errors
          return reply.status(400).send(
            wrapError(ErrorCodes.BAD_REQUEST, result.message)
          );
        }
    ...
    try {
      ...
      return reply.status(502).send(
        wrapError(
          ...
          `Failed to refresh registry: ${message}`
        )
      );
    }
  });
