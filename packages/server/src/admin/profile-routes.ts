...
export const registerAdminProfileRoutes: FastifyPluginCallback<AdminProfileRoutesConfig> = (
  ...
  fastify.get('/v1/admin/profiles/:profileId', async (request, reply) => {
    ...
    if (!profile) {
      return reply.status(404).send(wrapError(ErrorCodes.NOT_FOUND, 'Profile not found'));
    }
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.message.includes('cycle')) {
          return reply
            .status(400)
            .send(wrapError(ErrorCodes.BAD_REQUEST, 'Profile inheritance cycle detected'));
        }
        if (error.message.includes('depth')) {
          return reply
            .status(400)
            .send(wrapError(ErrorCodes.BAD_REQUEST, 'Profile inheritance depth limit exceeded'));
        }
        if (error.message.includes('Parent profile not found')) {
          return reply
            .status(400)
            .send(wrapError(ErrorCodes.BAD_REQUEST, 'Parent profile not found'));
        }
      }
      throw error;
    }
  ...
  fastify.delete('/v1/admin/profiles/:profileId', async (request, reply) => {
    ...
    if (referencingClients.length > 0) {
      return reply
        .status(409)
        .send(wrapError(ErrorCodes.CONFLICT, 'Cannot delete profile: clients are still using it'));
    }
