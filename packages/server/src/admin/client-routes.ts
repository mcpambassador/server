...
export const registerAdminClientRoutes: FastifyPluginCallback<AdminClientRoutesConfig> = (
  ...
  fastify.post('/v1/admin/clients', async (request, reply) => {
    ...
    if (!userRecord) {
      return reply.status(400).send(wrapError(ErrorCodes.BAD_REQUEST, 'User not found'));
    }

    if (userRecord.status !== 'active') {
      return reply.status(400).send(wrapError(ErrorCodes.BAD_REQUEST, 'User is not active'));
    }
    ...
    if (!profile) {
      return reply.status(400).send(wrapError(ErrorCodes.BAD_REQUEST, 'Profile not found'));
    }
  ...
  fastify.patch('/v1/admin/clients/:clientId', async (request, reply) => {
    ...
    if (!key) {
      return reply.status(404).send(wrapError(ErrorCodes.NOT_FOUND, 'Client key not found'));
    }
    ...
    if (statusChanged && body.status === 'revoked') {
      // Update key
      await compatUpdate(db, clients).set(updates).where(eq(clients.client_id, clientId));
