...
export const registerAdminSecurityRoutes: FastifyPluginCallback<AdminSecurityRoutesConfig> = (
  ...
  fastify.post('/v1/admin/rotate-hmac-secret', async (request, reply) => {
    ...
    try {
      ...
      return reply
        .status(500)
        .send(wrapError(ErrorCodes.INTERNAL_ERROR, 'Failed to rotate HMAC secret'));
    }
  });
  ...
  fastify.post('/v1/admin/rotate-credential-key', async (request, reply) => {
    ...
    if (!bodyResult.success) {
      return reply
        .status(400)
        .send(wrapError(ErrorCodes.VALIDATION_ERROR, 'Invalid request', bodyResult.error.issues));
    }
    ...
    try {
      ...
      const { updateCredential, user_mcp_credentials, compatSelect, compatTransaction } =
      ...
      return reply
        .status(500)
        .send(wrapError(ErrorCodes.INTERNAL_ERROR, 'Failed to rotate credential master key'));
    } finally {
