    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (!mcpEntry) {
          return reply
            .status(400)
            .send(wrapError(ErrorCodes.NOT_FOUND, `MCP '${mcp_name}' not found in catalog`));
        }
          ...
          return reply
            ...
            .send(
              wrapError(
                ...
                'OAuth configuration incomplete — required server environment variables are not set. Contact your administrator.'
              )
            );
        }
        ...
        return reply
          .status(500)
          .send(wrapError(ErrorCodes.INTERNAL_ERROR, 'Failed to generate authorization URL'));
      }
    }
  );
  ...
  fastify.get('/v1/oauth/callback', async (request, reply) => {
    ...
    try {
      ...
      const [existing] = await compatSelect(db)
        ...
        .where(
          and(eq(user_mcp_credentials.user_id, userId), eq(user_mcp_credentials.mcp_id, mcpId))
        )
        .limit(1);
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (!mcpEntry) {
          return reply
            .status(404)
            .send(wrapError(ErrorCodes.NOT_FOUND, `MCP '${mcpName}' not found in catalog`));
        }
  ...
  fastify.delete(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        // Parse oauth_config
        const oauthConfig =
          mcpEntry.oauth_config && mcpEntry.oauth_config !== '{}'
