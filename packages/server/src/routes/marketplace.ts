...
export async function registerMarketplaceRoutes(
  ...
  // ==========================================================================
  fastify.get('/v1/marketplace', { preHandler: requireUserSession }, async (request, reply) => {
    // Get user ID from session
    const userId = request.session.userId;
    if (!userId) {
      return reply.status(401).send(wrapError(ErrorCodes.UNAUTHORIZED, 'User session required'));
    }
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (!mcp) {
          return reply
            .status(404)
            .send(wrapError(ErrorCodes.NOT_FOUND, 'MCP not found or not accessible'));
        }
